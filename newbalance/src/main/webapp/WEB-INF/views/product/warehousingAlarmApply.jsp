		           						...
		           						<c:choose>
		       	  							<c:when test="${ dto.stockCount eq 0 }">
		       	  								 <li>
		         										...
		         										<input type="radio" id="${ dto.sz }" name="sizeRadio" value="${ dto.sz }">
		         										<label for="${ dto.sz }">${ dto.sz }</label>
		         										<input type="hidden" name="size" value="${ dto.sz }">
		         									</form>
		         								</li>
		       	  							</c:when>
		       	  							<c:otherwise>
		       	  								<li>
			         								<input type="radio" id="${ dto.sz }" name="sizeRadio" value="${ dto.sz }" disabled="disabled">
			         								<label for="${ dto.sz }">${ dto.sz }</label>
		         								</li>
		       	  							</c:otherwise>
		       	  						</c:choose>
...
<script>
	...
	$("form label").click(function(){
		...
		size = $(this).html();
		$("form").children("input[name=size]").val(size);
	});
