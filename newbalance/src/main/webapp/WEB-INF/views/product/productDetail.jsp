...
.modal_inner .inquiry.comp .comp_txt1::before {
    content: '';
    display: block;
    width: 100%;
    height: 87px;
    margin: 0 auto;
    color: #121212;
    background: url(https://image.nbkorea.com/NBRB_PC/common/icon_checkpoint.png) no-repeat center 0;
}
.modal_inner .comp_txt2 {
    color: #777;
    line-height: 20px;
...
#btnCloseProdQnaRegCompletePopup{
	right: 65px;
    top: 70px;
}
#prodQnaRegCompletePopup .modal_inner{
	padding: 30px 30px;
}
</style>
</head>
<body>
 ...
 function detailFixed(){
			...
			<div class="item_detailTop">
				...
				<div class="item_option">
					...
					<div class="box">
						...
						<div class="price">
							<!-- 금액 -->
							<fmt:formatNumber type="currency" value="${ pdDto.pdPrice }" pattern="###,###,###" var="price" />
					...
					<div class="op_select">
						...
						<div class="total_price">
							...
							<div class="cart_buy">
								...
								<a href="javascript:void(0);" class="buy" onclick="buy();">구매하기</a>
							</div>
						...
						<div class="option_wrap">
							...
							<div class="sec_prices">
								...
								<div class="total_btn">
									...
									<a href="javascript:void(0);" onlcick="buy();" class="buy md">구매하기</a>
								</div>
							</div>
									...
									<c:forEach var="dto" items="${ relatedPdList }"
										...
										<li class="swiper-slide"><a
											href="<%= contextPath %>/product/productDetail.action?pdCode=${ dto.pdCode }">
												<p class="thumbImg">
													<img src="${ dto.imgUrl }" alt="${ dto.pdName }" style="width: 164px; height: 164px;">
												</p>
												<div class="main_prdInfo">
													...
													<div class="main_prdPrice">
														...
														<del style="display: none">${ price }<span>원</span></del>
														<p>${ price }<span>원</span>
														</p>
													</div>
												</div>
	...
	<!-- 문의하기 모달창 -->
	<div class="dimm_pop" id="prodQnaPopupDimmed" style="position: fixed; display: none;"></div>
	<div class="modal" id="modalQna" style="position: absolute; display: none;">
		<div class="modal_inner">
			...
			<div class="contents">
				...
				<form name="formQna" id="formQna" enctype="multipart/form-data" method="post">
					<input type="hidden" name="pdCode" id="pdCode_qna" value="${ pdDto.pdCode }">
					<input type="hidden" name="colorCode" id="colorCode_qna" value="15">
					...
					<div class="form_area">
						<fieldset>
							...
							<div class="row">
								...
								<div class="fdata">
									<input type="text" id="qnaTitle" name="qnaTitle" class="ip_text"
										placeholder="문의의 종류를 간략히 적어주시면 빠른 답변에 도움이 됩니다." maxlength="30" value="">
								</div>
							</div>
							<div class="row">
								<label for="contents" class="ftit">문의 내용 <span
									class="necessary">필수</span></label>
								<div class="fdata">
									<textarea id="qnaContent" name="qnaContent" class="textarea"
										cols="30" rows="5" placeholder="문의하실 내용을 적어주세요." value=""></textarea>
								</div>
							</div>
							<div class="row">
								...
								<div class="fdata">
									<div class="file">
										<input id="qnaFile" type="text" value="" title="파일명" class="ip_text" name="qnaFile" readonly> 
										<span class="btn_attach_file">
											<input type="file" title="파일찾기" name="attachFile" id="attachFile" accept="image/jpg, image/gif"> 
					...
					<div class="btn_area">
						<a href="javascript:void(0);" class="btn_qna" id="btnWriteQna" onclick="addQna();">문의하기</a> 
						<a href="javascript:void(0);" class="btn_cancel" id="btnCancelQna">취소하기</a>
					</div>
				</form>
			</div>
	...
	<div class="modal" id="prodQnaRegCompletePopup" style="position: absolute; display: none;">
		<div class="modal_inner">
			<div class="square">
				<div class="inquiry comp">
					<strong class="comp_txt1">고객님의 문의가 정상적으로 접수되었습니다.</strong>
					<p class="comp_txt2">답변 완료 시 본 페이지나 <em class="point_b">‘마이페이지 &gt; 상품내역 &gt; 상품문의’</em>에서도 확인하실 수 있습니다.<br>항상 빠르고 친절하게 답변 드리도록 노력하겠습니다.</p>
					<div class="btn_area">
						<a href="#none" class="btn_ty_bface" id="btnProdQnaRegComplete">확인하기</a>
					</div>
				</div>
				<button type="button" class="close" id="btnCloseProdQnaRegCompletePopup">
					<img src="https://image.nbkorea.com/NBRB_PC/common/btn_pop_close.png" alt="팝업 닫기">
				</button>
			</div>
		</div>
	...
	<script>
			...
			if (total >= $("#maximum").html() || total >= stock) {
				...
				// 하단 셀렉터
  		 		var content2  = "<li class='clearfix2'>"
					...
					+ "<input name='pdPrice' id='pd_price' type='hidden' value='" + $('.box .price span b').html().replaceAll(",", "") + "'>"
					+ "<input name='pdCode' id='pd_code' type='hidden' value='" + $(this).siblings('input').data("pdcode") + "'>"
					+ "<input name='colorCode' id='color_code' type='hidden' value='" + $(this).siblings('input').data("colorcode") + "'>"
					+ "<input name='pdSize' id='pd_size' type='hidden' value='" + $(this).siblings('input').val() + "'>"
					+ "<input name='sizeCode' id='sizeCode' type='hidden' value='" + $(this).siblings('input').data("sizecode") + "'>"
					+ "<input name='pdAmount' id='pd_amout' type='hidden' value='1'>"
					+ "<input name='pdColor' id='pd_color' type='hidden' value='" + $(this).siblings('input').data("color") + "'>"
					+ "<input name='pdImage' id='pd_image' type='hidden' value='" + $("#main_img").prop("src") + "'>"
					+ "<input name='pdName' id='pd_name' type='hidden' value='" + $("#pdName").html() + "'></form></li>"
		...
		$("#size_code").change(function() {
			...
			if (total >= $("#maximum").html() || total >= stock) {
				...
				+ "<form method='post' name='buy_form' id='buy_form'>"
				+ "<input name='pdPrice' id='pd_price' type='hidden' value='" + $('.box .price span b').html().replaceAll(",", "") + "'>"
				+ "<input name='pdCode' id='pd_code' type='hidden' value='" + $(this).data("pdcode") + "'>"
				+ "<input name='colorCode' id='color_code' type='hidden' value='" + $(this).data("colorcode") + "'>"
				+ "<input name='pdSize' id='pd_size' type='hidden' value='" + $(this).val() + "'>"
				+ "<input name='sizeCode' id='sizeCode' type='hidden' value=''>"
				+ "<input name='pdAmount' id='pd_amout' type='hidden' value='1'>"  // acount
				+ "<input name='pdColor' id='pd_color' type='hidden' value='" + $(this).data("color") + "'>"
				+ "<input name='pdImage' id='pd_image' type='hidden' value='" + $("#main_img").prop("src") + "'>"
				+ "<input name='pdName' id='pd_name' type='hidden' value='" + $("#pdName").html() + "'></form></li>"
...
<script>
	...
	function addQna(){
		...
		if(usercode == ""){
			...
			if($("#qnaTitle").val() == "") { alert("제목을 입력해 주세요.") }
			else if($("#qnaContent").val() == "") { alert("문의하실 내용을 입력해 주세요.") }
			else{
				...
				if(result == true){
					$.ajax({
						...
						dataType : 'text',
						cache : false,
						data : $("#formQna").serialize(), 
						success : function(data){
							if(data == "00"){
								$("#modalQna").css("display", "none");
							    $("#prodQnaRegCompletePopup").css({
							        	"top": (($(window).height()-$("#prodQnaRegCompletePopup").outerHeight())/2+$(window).scrollTop())+"px",
							        	"display" : "block"
							    });
							    $(".dimm_pop").css("display", "block");
							}
							if(data == "99"){
						    	alert("상품 문의 등록에 실패하였습니다.");
						    }
						},
						error: function(data){
						    alert("에러가 발생했습니다.");
